...
app_server <- function( input, output, session ) {
  ...
  if (isFALSE(reactive({input$Q_filter_data}))) {
    n_metabolites = reactive({ncol(data_NMR$data_original())})
    n_samples <- reactive({nrow(data_NMR$data_original())})
    data_original <- reactive({data_NMR$data_original()})
    data_n <- reactive({data_NMR$data_n()})
    data_ns <- reactive({data_NMR$data_ns()})
  } else {
    ...
    data_original <- reactive({data_NMR$data_original()})
    n_metabolites = reactive({ncol(res_filter$filtered())})
    n_samples <- reactive({nrow(res_filter$filtered())})
    data_n <- reactive({res_filter$filtered()})
    data_ns <- reactive({
  ...
  output$datadesc <- shinydashboard::renderInfoBox({
    shinydashboard::infoBox(title = "DATA Description", color = "red", fill = T, icon = icon("chart-bar"),
  ...
  output$datatrans <- shinydashboard::renderInfoBox({
    ...
    shinydashboard::infoBox(title = "Normalisation and Scaling", color = "blue", fill = T, icon = icon("chart-bar"),
  ...
  mod_spectra_plot_server("spectra_plot_ui_1",
                          ...
                          data_NMR_n = data_n, 
                          data_NMR_ns = data_ns, 
                          index_metadata = reactive({data_NMR$index_metadata()}))
