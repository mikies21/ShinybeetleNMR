...
mod_Upload_data_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    return(
      list(
        "data_original" = reactive({upfile()}),
        "data_n" = reactive({data_n()}),
        "data_ns" = reactive({data_ns()}),
        "index_metadata" = reactive({input$metadata_colnames}),
        "normalisation" = reactive({input$normalisation}),
        "scaling" = reactive({input$scaling})
      )
    )
