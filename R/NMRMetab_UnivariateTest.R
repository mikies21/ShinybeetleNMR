...
NMRMetab_UnivarTest <- function(data, index_col = 3, group = "sampleID", paired = F, normality = T, equal.variance = F) {
  ...
  for (i in 1:ncol(dat)) {
    g1 <- dat[data[, group] == groups[1], i]
    g2 <- dat[data[, group] == groups[2], i]
